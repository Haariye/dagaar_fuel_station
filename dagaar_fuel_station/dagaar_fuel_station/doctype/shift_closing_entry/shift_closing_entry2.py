...
@frappe.whitelist()
def get_station_nozzles(company=None, pos_profile=None, include_existing=False, existing_nozzles=None):
    ...
    price_list = get_pos_price_list(pos_profile)
    out = []
        ...
        out.append(
            {
                ...
                "rate": get_item_rate(nozzle.item, price_list, nozzle.uom, company=company),
            }
        )
    return out
...
class ShiftClosingEntry(Document):
    ...
    def auto_add_station_nozzles(self):
        ...
        for nozzle in get_station_nozzles(self.company, self.pos_profile, include_existing=False, existing_nozzles=list(existing)):
